% This script finds the parameters of best fit from interval of 6 to 7
% weeks.

Nin = [Nsens(7), Nres(7)]; % 6 wks
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(8),Nres(8)]; % 7 wks
Nout_tot = sum(Nout);
...
%%
store_params7_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params7_sort = sortrows(store_params7_uns, 7);
store_params(:,:,7) = store_params7_sort;
store_acc_val(7,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

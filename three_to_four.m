% This script finds the parameters of best fit from interval of 3 to 4
% weeks.

Nin = [Nsens(4), Nres(4)]; % 3 wks
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(5),Nres(5)]; % 4 wks
Nout_tot = sum(Nout);
...
%%
store_params4_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params4_sort = sortrows(store_params4_uns, 7);
store_params(:,:,4) = store_params4_sort;
store_acc_val(4,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

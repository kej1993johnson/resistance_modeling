% This script finds the parameters of best fit from interval of 2 to 3
% weeks.

Nin = [Nsens(3), Nres(3)]; % 2 wks
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(4),Nres(4)]; % 3 wks
Nout_tot = sum(Nout);
...
%%
store_params3_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params3_sort = sortrows(store_params3_uns, 7);
store_params(:,:,3) = store_params3_sort;
store_acc_val(3,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

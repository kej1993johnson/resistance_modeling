% This script finds the parameters of best fit from interval of 7 to 8
% weeks.

Nin = [Nsens(8), Nres(8)]; % 7 wks
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(9),Nres(9)]; % 8 wks
Nout_tot = sum(Nout);
...
%%
store_params8_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params8_sort = sortrows(store_params8_uns, 7);
store_params(:,:,8) = store_params8_sort;
store_acc_val(8,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

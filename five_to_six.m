% This script finds the parameters of best fit from interval of 5 to 6
% weeks.

Nin = [Nsens(6), Nres(6)]; % 5 wks
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(7),Nres(7)]; % 5 wks
Nout_tot = sum(Nout);
...
%%
store_params6_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params6_sort = sortrows(store_params6_uns, 7);
store_params(:,:,6) = store_params6_sort;
store_acc_val(6,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

% This script finds the parameters of best fit from interval of 1 to 2
% weeks.

Nin = [Nsens(2), Nres(2)]; % 1 wk
Nin_tot = sum(Nin);
ratio_in = Nin(1)./Nin(2); 
Nout = [Nsens(3),Nres(3)]; % 2 wks
Nout_tot = sum(Nout);
...
%%
store_params2_uns = (horzcat(ksre', krse', gse', gre', Nout_guess_acc(:,1), Nout_guess_acc(:,2), err_all_newe'));
store_params2_sort = sortrows(store_params2_uns, 7);
store_params(:,:,2) = store_params2_sort;
store_acc_val(2,:) = horzcat(ksr1, krs2, gs1, gr1, Nout_guess1(1), Nout_guess1(2), err_all_curr);
%%
figure(1)
hold off

...
[beta1_8, resnorm1_8, residuals1_8, output1, lambda1, Jacobian1] = lsqnonlin(@fit_simp1popunw, initials1new,[0; 0],[ Inf; Inf],[],dose,var, wknum, Vmaxall);
X = (0:.1:250);
X = X';
v_model1=model1pop( dose, beta1_8, Vmaxweekavg);
v_model1_naive = model1pop(dosenc, betaLD50naive_8, Vmaxnaiveavgcohort);
v_model1plot = model1pop(X, beta1_8, Vmaxweekavg);
...
[lowerlim1boot, upperlim1boot] = BSerrorinparams1(residuals1_8, dose, beta1_8, Vmaxweekavg, cohort_8);
...
beta1_8unt = load('beta1_8unt.mat');
beta1_8unt = struct2cell(beta1_8unt);
beta1_8unt = cell2mat(beta1_8unt);
v_model1plotunt = model1pop(X, beta1_8unt, Vmaxweekavg);
...
plot(X, v_model1plotunt, '-b', 'LineWidth',3);
xlim([0,250]);
xlabel('Dose (uM)','FontSize',18)
ylabel('Viability','FontSize',18)
title ('Single Population Model Cohort 8','FontSize',18)
legend('Experimental Treated Viability','Treated Single Static Population Model', 'Untreated Single Static Population Model')
%% Try to export data to csv for use in R
filenamedata = 'cohort8.csv';
filenameplot = 'cohort8plot.csv';
fileID = fopen(filename, 'wt');

labels = {'time' 'dose', 'viability','cohort','X', 'treated.static.model','untreated.static.model'};
% write string to csv
[rows,columns] = size(labels);
for index = 1:rows
    fprintf(fileID, '%s,', labels{index, 1:end-1});
    fprintf(fileID, '%s\n',labels{index,end});
end
fclose(fileID)

cohort8data = [ wk, dose, var, cohort_number]; 
cohort8plot = [X, v_model1plot, v_model1plotunt];
csvwrite('cohort8data.csv', cohort8data);
export(cohort8data,'File','cohort8data.csv','Delimiter',',')
%csvwrite(filenameplot, cohort8plot);
%%
figure(2)
hold off
...
% outputs betaLD50 with first 12 paramters slope values, second 12 are
% center values in week order
[betaLD50_8, resnormLD50_8, residualsLD50s_8] = lsqnonlin(@fit_simpLD5015,...
    params0,...
    paramslb,...
    paramsub,...
    options,...
...
bulk_model_LD50_8 = model_bulk_LD50( dose, betaLD50_8, Vmaxall, wk );
...
%% Perform Boot Strapping Estimates of Error in Parameters
[lowerlimLD50boot, upperlimLD50boot] = BSerrorinparamsLD50(residualsLD50s_8, cohort_8, betaLD50_8);

%% Print Boot strapping limits
lowerlimLD50boot8 = lowerlimLD50boot
upperlimLD50boot8 = upperlimLD50boot
errorbarlengthLD50boot8 = upperlimLD50boot-lowerlimLD50boot

%% Plot of LD50s over time with bootstrapping error

betaLD50_8unt = load('betaLD50_8unt.mat');
betaLD50_8unt = struct2cell(betaLD50_8unt);
betaLD50_8unt = cell2mat(betaLD50_8unt);

errorbarlengthLD508untreated = load('errorbarlengthLD50boot8untreated.mat');
errorbarlengthLD508untreated = struct2cell(errorbarlengthLD508untreated);
errorbarlengthLD508untreated = cell2mat(errorbarlengthLD508untreated);



figure (3)
errorbar(time, betaLD50_8(16:30),errorbarlengthLD50boot8(16:30)./2, 'ro-', 'LineWidth',2)
hold on
errorbar(time, betaLD50_8unt(16:30), errorbarlengthLD508untreated(16:30),'bo-', 'LineWidth',2)
errorbar(0, betaLD50naive_8(2), errorbarlength1bootnaive(2),'ro', 'LineWidth',2)
...
legend('Treated Single Population Response', 'Untreated Control Single Population Response', 'Naive Response')
...
%  res2sq=ones([n 1]);
[beta2_8, resnorm2_8, residuals2_8] = lsqnonlin(@fit_simp2popunw15,...
    params0,...
    paramslb,...
    paramsub,...
    options,...
...
v_model2allweeksnormed_8 = model2popallweeksnormed( dose, beta2_8, Vmaxbyweek, nsize);
...
beta2_8unt = load('beta2_8unt.mat');
beta2_8unt = struct2cell(beta2_8unt);
beta2_8unt = cell2mat(beta2_8unt);

errorbarlength2boot8unt = load('errorbarlength2boot8unt.mat');
errorbarlength2boot8unt = struct2cell(errorbarlength2boot8unt);
errorbarlength2boot8unt = cell2mat(errorbarlength2boot8unt);
...
[lowerlim2boot8, upperlim2boot8] = BSerrorinparams2(residuals2_8, beta2_8, cohort_8);
...
errorbarlength2boot8 = upperlim2boot8-lowerlim2boot8
...
hold on
errorbar(time, fres2unt, errorbarlength2boot8unt(5:19)./2, 'ro-', 'LineWidth', 2)
errorbar(0, fres0, errorbarlength2bootnaive8./2,'bo','LineWidth',2)
...
ylabel('Fraction of Resistant Cells', 'FontSize',14)
title ('Two Population Model Cohort 8', 'FontSize', 14)
legend ('Fraction of Treated Cells Resistant', 'Fraction of Untreated Cells Resistant', 'Fraction of Naive Cells Resistant')
hold off

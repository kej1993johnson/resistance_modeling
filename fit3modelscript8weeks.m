...
% Does this for only 8 weeks of data-> not finished, need to also rerun
% untreated on only eight weeks of data
...
% Remove Cohort 3
coh_ind = vddata(:, 4) == 3;
vddata = vddata(coh_ind == 0, :); % time (weeks), dose, viability, cohort number
%coh_ind8 = vddata(:,4) == 8;
...
[lowerlim1boot, upperlim1boot, AICvec1, MSEvec1] = BSerrorinparams1(residuals1, dose, beta1new, Vmaxweekavg, vddata);
%%
AIC1_CI = prctile(AICvec1, [2.5 97.5])
 AIC1_mean = mean(AICvec1)
 lowerlimAIC1= AIC1_CI(1)
 upperlim1AIC1 = AIC1_CI(2)
  MSE1_CI = prctile(MSEvec1, [2.5 97.5])
 MSE1_mean = mean(MSEvec1)
 lowerlimMSE1= MSE1_CI(1)
 upperlim1MSE1 = MSE1_CI(2)
%%
...
params1 = [ beta1new', lowerlim1boot, upperlim1boot, betaLD50naive', lowerlim1bootnaive, upperlim1bootnaive];
        param_single_static_tbl = dataset({params1,'slope','LD50single', 'slope_low', 'LD50low', 'slope_high', 'LD50_high', 'slope_naive', 'LD50_naive', 'slope_naive_low', 'LD50naive_low', 'slope_naive_high,', 'LD50naive_high'});
        save('.param_single_static_tbl.mat', 'param_single_static_tbl')
...
legend('Treated','Untreated','Treated LD50 = 50.4 +/-2.4 uM', 'Untreated LD50 = 37.0 +/- 3.5 uM')
legend boxoff
...
% Plot of LD50s over time
%plot(time, betaLD50(16:30), 'o', 'LineWidth', 4)
errorbar(time, LD50s-LD50sunt, errorbarsLD50./2, '-go', 'LineWidth', 2)
hold on
%errorbar(0, betaLD50naive(2), errorbarlength1bootnaive(2)./2,'ko', 'LineWidth',2)
%plot(0, betaLD50naive(2), 'go', 'LineWidth',4)
xlabel ('Time Post Treatment(weeks)', 'FontSize', 16)
ylabel ('Difference in LD50 (uM)', 'FontSize', 16)
set(gca,'LineWidth',1.5,'FontSize',16)
%errorbar(time, LD50sunt, errorbarsLD50unt./2,'ko', 'LineWidth',2)

title('Single Dynamic Population Model','FontSize', 18)
legend('Normalized Treated Single Population LD50')
...
% Plot of LD50s over time
%plot(time, betaLD50(16:30), 'o', 'LineWidth', 4)
errorbar(time, betaLD50(9:16), errorbarsLD50(2:9), 'b-', 'LineWidth', 2)
hold on
...
plot(dose, var, '.', dose, v_model2allweeksnormed, 'o')
%2D view
figure(10)
hold off
plot(dose, var, 'bo')
...
set(gca,'LineWidth',1.5,'FontSize',12)
legend ('Fraction of Treated Cells Resistant LD50 = 79.7 uM', 'Fraction of Untreated Cells Resistant')
hold off
...
hold on
plot (dose(sum(nsize(1:7))+1:sum(nsize(1:8))), var(sum(nsize(1:7))+1:sum(nsize(1:8))), 'go')
...
xlim ([0 220])
xlabel('dose (uM)','FontSize',10)
ylabel('Viability','FontSize',10)
% title ('Two Population Model 3, 6 & 10 Weeks Post Treatment','FontSize',10)
